---
name: mem
description: "Universal memory operations — search, retrieval, dedup. Explicit: user asks about past work, previous decisions, \"did we already do this\". Implicit: proactively check before re-deriving conclusions when researching a topic that may have prior work."
---
...
### @mem search [query]
Perform a semantic and keyword search. Two-layer fallback:

**Layer 1 — claude-mem (structured observations):**
...
- If results are found and relevant, return them.

**Layer 2 — transcript search (fallback):**
- If Layer 1 returns nothing useful, run: `python3 ~/.claude/search-transcripts.py "[query]"`
- Searches full verbatim JSONL transcripts across all sessions (365-day retention).
- Returns matching user messages + surrounding assistant context.
- Use when: "we discussed X before", "you researched Y", "what did we decide about Z"
