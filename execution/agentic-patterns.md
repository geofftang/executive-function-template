...
Living reference for automation and agentic design thinking. Organized by the 8-Check audit (matches `execution/skills/plan/SKILL.md`). Grows as patterns are discovered and validated.
...
### Ralph Loop (Iterative Autonomous Execution)
Two architectures for the same pattern — AI iterates on a task until a completion condition is met:

- **Bash loop** (`while true; do cat PROMPT.md | claude -p; done`) — Fresh context each iteration. Best when: iterations are independent, context bloat is a risk, work is decomposable into checkboxed tasks. Each iteration reads spec + implementation plan + picks next unchecked task.
- **Stop hook loop** (official `ralph-loop` plugin) — Stays in-session, Claude sees its own file changes across iterations. Best when: iterations build on each other, continuity matters more than freshness. Uses a Stop hook that blocks exit until a completion promise (`<promise>DONE</promise>`) is detected.

Both require: well-defined completion criteria, work that can be verified automatically (tests, linters, eval checks), and a spec/plan written before the loop starts. See `@plan > When Principle 5 Fires` for the full spec production workflow.
